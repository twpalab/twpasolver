...
@nb.njit
def _filter_wm_terms(
    kappas,
    relations_3wm,
    relations_4wm,
    relative_deltak_cutoff
):
    ...
    for k3wm in range(len_3wm):
        ...
        for sign_idx in range(4):
            ...
            if np.abs(dkappa / base_kappa) < relative_deltak_cutoff:
                ...
                valid_3wm[num_valid_3wm, 1] = rel_3wm[1] + (2 * num_modes if s1 == -1 else 0)
                valid_3wm[num_valid_3wm, 2] = rel_3wm[2] + (2 * num_modes if s2 == -1 else 0)
                num_valid_3wm += 1
    ...
    for k4wm in range(len_4wm):
        ...
        for sign_idx in range(8):
            ...
            if np.abs(dkappa / base_kappa) < relative_deltak_cutoff:
                ...
                valid_4wm[num_valid_4wm, 1] = rel_4wm[1] + (2 * num_modes if s1 == -1 else 0)
                valid_4wm[num_valid_4wm, 2] = rel_4wm[2] + (2 * num_modes if s2 == -1 else 0)
                valid_4wm[num_valid_4wm, 3] = rel_4wm[3] + (2 * num_modes if s3 == -1 else 0)
                num_valid_4wm += 1
...
def general_cme_first_round(
    ...
    gauss = np.exp(-(x-26500/2)**2/(2*(5000)**2))
...
def general_cme_with_backward(
    ...
    for i in range(num_modes):
        alphas_rhs[i] = currents[i] * exp_pos[i]
        alphas_rhs[i + num_modes] = np.conj(alphas_rhs[i])
        alphas_rhs[i + 2*num_modes] =  (np.interp(x, x_evals, currents_evals[i].real)
                + 1j * np.interp(x, x_evals, currents_evals[i].imag))* exp_neg[i]
...
def cme_general_solve_freq_array_fb_cutoff(
    ...
    for i in nb.prange(n_freq):
        ...
        relations_3wm_optimized, relations_4wm_optimized = _filter_wm_terms(gammas_array[i].imag, relations_3wm, relations_4wm, cutoff)
...
def cme_solve_forward_backward_cut(
    x_array: FloatArray,
    y0_array_fwd: ComplexArray,
    y0_array_bwd: ComplexArray,
