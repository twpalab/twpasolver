    ...
    for i in nb.prange(n_freq):
        ...
        for k in range(n_passes):
            if k == 0:
                ...
                if k > 2:
                    ...
                    for j in range(n_modes):
                        current_ratio_bwd = A_bwd[n_modes + j, 0] / (
                            gammas_array[i, j] * A_bwd[j, 0]
                        )
                        current_ratio_fwd = A_fwd[n_modes + j, -1] / (
                            gammas_array[i, j] * A_fwd[j, -1]
                        )
                        reflection_coeff[j] = (
                            Zb[i, j] * (1 - current_ratio_bwd) - Z0
                        ) / (Zb[i, j] * (1 + current_ratio_fwd) + Z0)
                else:
            ...
            if k > 2:
                reflection_coeff = np.empty(n_modes, dtype=np.complex128)
                for j in range(n_modes):
                    current_ratio_bwd = A_fwd[n_modes + j, 0] / (
                        gammas_array[i, j] * A_fwd[j, 0]
                    )
                    current_ratio_fwd = A_bwd[n_modes + j, -1] / (
                        gammas_array[i, j] * A_bwd[j, -1]
                    )
                    reflection_coeff[j] = (Zb[i, j] * (1 - current_ratio_bwd) - Z0) / (
                        Zb[i, j] * (1 + current_ratio_fwd) + Z0
                    )
            else:
                reflection_coeff = reflections_array[i]
