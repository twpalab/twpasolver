...
def plot_quantum_efficiency(
    ...
    Args:
        ...
        freqs_unit: Label for the frequency axis.
        figsize: Overall figure size.
        lw: Line width.

    Returns:
        Array of Axes objects, shape (4,).
    """
    ...
    noise_modes = mode_info.get("noise_modes", list(noise_channels.keys()))

    n_modes = len(gain_db_per_mode)
    ...
    ax = axes[0]
    for i, (label, g_db) in enumerate(gain_db_per_mode.items()):
        lw_i = lw * 1.4 if label == signal_mode else lw
        ax.plot(freqs, g_db, label=label, color=colors[i], ls=ls_cycle[i], lw=lw_i)
    ax.set_ylabel("Gain [dB]")
